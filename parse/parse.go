...
// and html/template.
//
// This is a copy of the parse package from text/template which doesn't error
// out if functions are not defined, making it possible to write tools for
// templates.
package parse
...
const (
	...
	ParseRelaxFunctions                  // parse won't error out if a function is not defined.
)
...
func Parse(name, text string, mode Mode, leftDelim, rightDelim string, funcs ...map[string]interface{}) (map[string]*Tree, error) {
	treeSet := make(map[string]*Tree)
	t := New(name)
	t.text = text
	t.Mode = mode
	_, err := t.Parse(text, mode, leftDelim, rightDelim, treeSet, funcs...)
	return treeSet, err
}
...
// the treeSet map.
func (t *Tree) Parse(text string, mode Mode, leftDelim, rightDelim string, treeSet map[string]*Tree, funcs ...map[string]interface{}) (tree *Tree, err error) {
	defer t.recover(&err)
	t.ParseName = t.Name
...
func (t *Tree) term() Node {
	switch token := t.nextNonSpace(); token.typ {
	case itemIdentifier:
		if t.Mode&ParseRelaxFunctions == 0 && !t.hasFunction(token.val) {
