...
func testParse(doCopy bool, t *testing.T) {
	...
	for _, test := range parseTests {
		tmpl, err := New(test.name).Parse(test.input, 0, "", "", make(map[string]*Tree), builtins)
		switch {
...
func TestParseWithComments(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			tmpl, err := tr.Parse(test.input, 0, "", "", make(map[string]*Tree))
			if err != nil {
				t.Errorf("%q: expected error; got none", test.name)
			}
...
func TestIsEmpty(t *testing.T) {
	...
	for _, test := range isEmptyTests {
		tree, err := New("root").Parse(test.input, 0, "", "", make(map[string]*Tree), nil)
		if err != nil {
			t.Errorf("%q: unexpected error: %v", test.name, err)
			continue
		}
...
func TestErrorContextWithTreeCopy(t *testing.T) {
	tree, err := New("root").Parse("{{if true}}{{end}}", 0, "", "", make(map[string]*Tree), nil)
	if err != nil {
...
func TestErrors(t *testing.T) {
	for _, test := range errorTests {
		t.Run(test.name, func(t *testing.T) {
			_, err := New(test.name).Parse(test.input, 0, "", "", make(map[string]*Tree))
			if err == nil {
...
func TestBlock(t *testing.T) {
	...
	tmpl, err := New("outer").Parse(input, 0, "", "", treeSet, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLineNum(t *testing.T) {
	...
	tree, err := New("bench").Parse(text, 0, "", "", make(map[string]*Tree), builtins)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseRelaxFunctions(t *testing.T) {
	tr, err := Parse("", "{{add 1 2}}", ParseRelaxFunctions, "{{", "}}", nil)
	if err != nil {
		t.Fatal(err)
	}
	if err != nil {
...
func BenchmarkParseLarge(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := New("bench").Parse(text, 0, "", "", make(map[string]*Tree), builtins)
		if err != nil {
			b.Fatal(err)
		}
	}
